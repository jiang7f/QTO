...
# 单次提交电路可能不能超过100，要分批
flp_problems_pkg, flp_configs_pkg = generate_flp(num_cases, [(3, 3)], 1, 10)
gcp_problems_pkg, gcp_configs_pkg = generate_gcp(num_cases, [(3, 2)])
kpp_problems_pkg, kpp_configs_pkg = generate_kpp(num_cases, [(5, 3, 4)], 1, 20)
jsp_problems_pkg, jsp_configs_pkg = generate_jsp(num_cases, [(2, 3, 4)], 1, 20)
scp_problems_pkg, scp_configs_pkg = generate_scp(num_cases, [(5, 5)])
...
def process_layer(prb, num_layers, solver,  backend, shots, shared_cloud_manager):
    try:
        opt = CobylaOptimizer(max_iter=70)
