...
kpp_problems_pkg, kpp_configs_pkg = generate_kpp(num_cases, [(4, 2, 3)], 5, 20)
jsp_problems_pkg, jsp_configs_pkg = generate_jsp(num_cases, [(2, 2, 3)], 5, 20)


problems_pkgs = [flp_problems_pkg, kpp_problems_pkg ,jsp_problems_pkg]


configs_pkg = flp_configs_pkg + kpp_configs_pkg + jsp_configs_pkg
...
def process_layer(prb, num_layers, solver,  backend, shots):
    try:
        opt = CobylaOptimizer(max_iter=40)
        # provider = FakeTorinoProvider()
...
if __name__ == '__main__':
    ...
    set_timeout = 60 * 60 * 24 * 4 # Set timeout duration
    num_complete = 0
    print(new_path)
    ...
    with ProcessPoolExecutor(max_workers=num_processes_cpu - 2) as executor:
        futures = []
        for pksid, problems_pkg in enumerate(problems_pkgs):
                        ...
                        for pbid, prb in enumerate(problems):
                            for solver in solvers:
                                if solver == ChocoSolver:
                                    ...
                                    layer = 5
                                print(f'{pksid, pkid, pbid, solver.__name__, backend, shots} build')
                                future = executor.submit(process_layer, prb, layer, solver, backend, shots)
                                futures.append((future, prb, pksid, pkid, pbid, layer, solver.__name__))

        start_time = time.perf_counter()
        for future, prb, pksid, pkid, pbid, layer, solver in futures:
            ...
            try:
                ...
                diff.extend(metrics)
                print(f"Task for problem {pksid}-{pkid}-{pbid} L={layer} {solver} executed successfully.")
            except MemoryError:
                print(f"Task for problem {pksid}-{pkid}-{pbid} L={layer} {solver} encountered a MemoryError.")
                for dict_term in evaluation_metrics:
                    diff.append('memory_error')
            except TimeoutError:
                print(f"Task for problem {pksid}-{pkid}-{pbid} L={layer} {solver} timed out.")
                for dict_term in evaluation_metrics:
                    diff.append('timeout')
            except Exception as e:
                print(f"An error occurred: {e}")
            finally:
                row = [pksid, pkid, pbid, layer, len(prb.variables), len(prb.lin_constr_mtx), solver] + diff
