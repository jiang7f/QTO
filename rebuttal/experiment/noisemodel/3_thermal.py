...
from qto.solvers.qiskit import (
    ...
    QtoSimplifyDiscardSegmentedSolver,BitFlipNoiseAerProvider,DepolarizingNoiseAerProvider,ThermalNoiseAerProvider,
...
headers = ['pkid', 'pbid', 'layers', "variables", 'constraints', 'method', 'T2'] + evaluation_metrics
new_dir = os.path.dirname(new_path)
if not os.path.exists(new_dir):
    os.makedirs(new_dir)

def process_layer(prb, num_layers, solver,t, pkid, pbid):
    opt = CobylaOptimizer(max_iter=200, save_address = f'rebuttal/data/noisemodel/loss/{pkid}-{pbid}-{num_layers}-{solver.__name__}-p_gate1={t2}.csv')
    aer = ThermalNoiseAerProvider(t1=2*t,t2=t)
    prb.set_penalty_lambda(200)
    used_solver = solver(
        prb_model = prb,
        optimizer = opt,
        provider = aer,
...
if __name__ == '__main__':
    ...
    # + gcp_configs_pkg + kpp_configs_pkg + jsp_configs_pkg + scp_configs_pkg
    ...
    t2_list = [50e3,75e3,100e3,150e3,200e3,300e3,500e3,1e6,2e6,5e6]
            ...
            with ProcessPoolExecutor(max_workers=num_processes) as executor:
                ...
                for t2 in t2_list:
                    for pbid, prb in enumerate(problems):
                        print(f'{pkid}-{pbid}, {layer}, {solver} t2={t2} is being build')
                        sys.stdout.flush()
                        # process_layer(prb, layer, solver, p)
                        future = executor.submit(process_layer, prb, layer, solver, t2 ,pkid, pbid)
                        futures.append((future, prb, pkid, pbid, layer, solver.__name__,t2))

                start_time = time.perf_counter()
                for future, prb, pkid, pbid, layer, solver,t2 in futures:
                    ...
                    try:
                        ...
                        diff.extend(metrics)
                        print(f"Task for problem {pkid}-{pbid} L={layer} {solver} p_gate1={t2} has executed successfully.")
                    except MemoryError:
                        print(f"Task for problem {pkid}-{pbid} L={layer} {solver} p_gate1={t2} encountered a MemoryError.")
                        for dict_term in evaluation_metrics:
                            diff.append('memory_error')
                    except TimeoutError:
                        print(f"Task for problem {pkid}-{pbid} L={layer} {solver} p_gate1={t2} timed out.")
                        for dict_term in evaluation_metrics:
                            diff.append('timeout')
                    except Exception as e:
                        print(f"An error occurred: {e}")
                    finally:
                        row = [pkid, pbid, layer, len(prb.variables), len(prb.lin_constr_mtx), solver, t2] + diff
