...
from qto.solvers.qiskit import (
    ...
    QtoSimplifyDiscardSegmentedSolver,BitFlipNoiseAerProvider,DepolarizingNoiseAerProvider,
...
def process_layer(prb, num_layers, solver,p, pkid, pbid):
    opt = CobylaOptimizer(max_iter=200, save_address = f'rebuttal/data/noisemodel/loss/{pkid}-{pbid}-{num_layers}-{solver.__name__}-p_gate1={p}.csv')
    aer = DepolarizingNoiseAerProvider(p_single_gate=p)
